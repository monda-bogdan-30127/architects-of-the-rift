...
export function evaluateTeamIdentity(team: Team | null | undefined, compIdentity: TeamDraftEvaluation["compIdentity"], champions: Array<Champion | null>): TeamIdentityEvaluation {
  ...
  const matchedTags = identity.preferredTags.filter((tag) => teamTags.has(tag));
  const avoidedTags = (identity.avoidTags ?? []).filter((tag) => teamTags.has(tag));
  ...
  const profileBlind = profiles.length ? profiles.reduce((sum, profile) => sum + profile.draftProfile.blindPick, 0) / profiles.length : 5;
  const profileFlex = profiles.length ? profiles.reduce((sum, profile) => sum + profile.draftProfile.flexValue, 0) / profiles.length : 0;
  const profileContest = profiles.length ? profiles.reduce((sum, profile) => sum + profile.draftProfile.contestPriority, 0) / profiles.length : 0;
  ...
  if (styleMatched) noteParts.push(`preferred style matched (${compIdentity})`);
  if (matchedTags.length) noteParts.push(`matched tags: ${matchedTags.join(", ")}`);
  if (avoidedTags.length) noteParts.push(`avoid tags hit: ${avoidedTags.join(", ")}`);
